...
const LanguageSwitcher: React.FC = () => {
    ...
    return (
        <div className="relative group">
            <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
                <Globe className="w-4 h-4" />
                <span>{i18n.language === 'uz' ? 'O\'zbek' : 'English'}</span>
            </button>
            <div className="absolute right-0 pt-2 w-full opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
